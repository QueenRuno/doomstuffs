...
class Protector:HDShotgun{
	default{
		...
		scale 0.57;
		inventory.pickupmessage "uhh";
		hdweapon.barrelsize 40,0.5,2;
		hdweapon.refid "pro";
		tag "protector shotgun";
		obituary "Hunted";
	...
	override string pickupmessage(){
		...
		if(weaponstatus[0]&PROTF_CANFULLAUTO)return msg..Stringtable.Localize("$PICKUP_HUNTER_AUTO");
		if(weaponstatus[0]&PROTF_EXPORT)return msg..Stringtable.Localize("$PICKUP_HUNTER_EXPORT");
		return msg;
	}
	...
	override void DrawHUDStuff(HDStatusBar sb,HDWeapon hdw,HDPlayerPawn hpl){
		...
		if(!(hdw.weaponstatus[0]&PROTF_EXPORT))sb.drawwepcounter(hdw.weaponstatus[HUNTS_FIREMODE],
	...
	override string gethelptext(){
		...
		..(weaponstatus[0]&PROTF_EXPORT?"":(LWPHELP_FIREMODE..StringTable.Localize("$SHTG_FMODE")..(weaponstatus[0]&PROTF_CANFULLAUTO?"/Auto":"").."\n"))
	...
	override double weaponbulk(){
		return 110+(weaponstatus[SHOTS_SIDESADDLE]+weaponstatus[HUNTS_TUBE])*ENC_SHELLLOADED;
	}
	action void A_SwitchFireMode(bool forwards=true){
		if(invoker.weaponstatus[0]&PROTF_EXPORT){
			invoker.weaponstatus[HUNTS_FIREMODE]=0;
			return;
		}
		int newfm=invoker.weaponstatus[HUNTS_FIREMODE]+(forwards?1:-1);
		int newmax=(invoker.weaponstatus[0]&PROTF_CANFULLAUTO)?2:1;
	...
	action void A_SetAltHold(bool which){
		if(which)invoker.weaponstatus[0]|=PROTF_ALTHOLDING;
		else invoker.weaponstatus[0]&=~PROTF_ALTHOLDING;
	}
	...
	action void A_CheckPocketSaddles(){
		if(invoker.weaponstatus[SHOTS_SIDESADDLE]<1)invoker.weaponstatus[0]|=PROTF_FROMPOCKETS;
		if(!countinv("HD16Gauge275InchShellAmmo"))invoker.weaponstatus[0]&=~PROTF_FROMPOCKETS;
	}
	...
	action bool A_GrabShells(int maxhand=3,bool settics=false,bool alwaysone=false){
		...
		bool fromsidesaddles=!(invoker.weaponstatus[0]&PROTF_FROMPOCKETS);
		int toload=min(
			fromsidesaddles?invoker.weaponstatus[SHOTS_SIDESADDLE]:countinv("HD16Gauge275InchShellAmmo"),
	...
	hold:
		SHTG A 0{
			...
			if(
				paf&&!(invoker.weaponstatus[0]&PROTF_ALTHOLDING)
			)setweaponstate("chamber");
			else if(!paf)invoker.weaponstatus[0]&=~PROTF_ALTHOLDING;
		}
	...
	altfire:
	chamber:
		SHTG A 0 A_JumpIf(invoker.weaponstatus[0]&PROTF_ALTHOLDING,"nope");
	...
	racked:
		...
		SHTG F 0{
			...
			if(pressingreload()){
				...
				invoker.weaponstatus[0]&=~PROTF_FROMPOCKETS;
				else{
					invoker.weaponstatus[0]|=PROTF_FROMPOCKETS;
					rld=2;
				}
	...
	rackreload:
		...
		SHTG F 0 A_JumpIf(!(invoker.weaponstatus[0]&PROTF_FROMPOCKETS),"rackloadone");
	...
	chamberauto:
		SHTG A 1 A_Chamber();
		SHTG A 1 A_JumpIf(invoker.weaponstatus[0]&PROTF_CANFULLAUTO&&invoker.weaponstatus[HUNTS_FIREMODE]==2,"ready");
		SHTG A 0 A_Refire();
		goto ready;
	flash:
		SHTF B 1 bright{
	...
	reloadfrompockets:
		SHTG A 0{
			...
			else invoker.weaponstatus[0]|=PROTF_FROMPOCKETS;
		}goto startreload;
	reload:
	...
	reloadstarthand:
		...
		SHTG H 0 A_JumpIf(invoker.weaponstatus[0]&PROTF_FROMPOCKETS,"reloadpocket");
	reloadfast:
	...
	reloadashell:
		...
		SHTG HHHHHH 1 offset(0,33){
			if(
				...
				||PressingZoom()
				||PressingFiremode()
			)invoker.weaponstatus[0]|=PROTF_HOLDING;
			else invoker.weaponstatus[0]&=~PROTF_HOLDING;
	...
	reloadend:
		...
		SHTG HGA 3;
		SHTG A 0 A_JumpIf(invoker.weaponstatus[0]&PROTF_HOLDING,"nope");
		goto ready;
	...
	unloadloopend:
		...
		SHTG H 3 offset(1,42){
			int rmm=HDPickup.MaxGive(self,"HD16Gauge275InchShellAmmo",ENC_SHELL);
			if(rmm>0){
				A_StartSound("weapons/pocket",9);
				A_SetTics(8);
				HDF.Give(self,"HD16Gauge275InchShellAmmo",min(rmm,invoker.handshells));
	...
	override void loadoutconfigure(string input){
		...
		if(type>=0){
			switch(type){
			case 0:
				weaponstatus[0]|=PROTF_EXPORT;
				weaponstatus[0]&=~PROTF_CANFULLAUTO;
				break;
			case 1:
				weaponstatus[0]&=~PROTF_EXPORT;
				weaponstatus[0]&=~PROTF_CANFULLAUTO;
...
class ProtectorRandom:IdleDummy{
	states{
	spawn:
		TNT1 A 0 nodelay{
			...
			if(!random(0,32)){
				ggg.weaponstatus[0]&=~PROTF_EXPORT;
				ggg.weaponstatus[0]|=PROTF_CANFULLAUTO;
			}else if(!random(0,7)){
				ggg.weaponstatus[0]|=PROTF_EXPORT;
				ggg.weaponstatus[0]&=~PROTF_CANFULLAUTO;
			}
			int tubesize=((ggg.weaponstatus[0]&PROTF_EXPORT)?5:7);
